...
GUI 的“音频转换”页可选择 FFmpeg 支持的媒体、音轨、质量模式、dB 增益和心理声学掩蔽；增益可直接输入或按 0.5 dB 步进，并显示对应线性倍率。“资源包”页可生成当前模式或全部推荐模式。耗时任务在后台执行，结果路径和错误会显示在底部日志中。重新执行 `pip install -e .` 后，也可直接运行 `wav2mc-gui`。
