...
The installable package uses a `src` layout. Production code lives in `src/wav2mc/`: `audio.py` handles decoding, `analysis.py` extracts tonal and residual components, `grains.py` defines deterministic hybrid grains, `preview.py` reconstructs audio, and `loudness.py` calibrates Minecraft volume. `bank.py`/`datapack.py` generate packs, `pipeline.py` coordinates conversion, `cli.py` defines commands, and `gui.py` provides the Tkinter desktop UI. Keep shared defaults and data classes in `config.py`.
